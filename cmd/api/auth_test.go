...
func Test_app_getTokenFromHeaderAndVerify(t *testing.T) {
	...
	var tests = []struct {
		...
		// wrong issuer test MUST run last
	...
	for _, e := range tests {
		...
		if err != nil && !e.errorExpected {
			t.Errorf("%s: did not an expect an error, but got one - %s", e.name, err.Error())
		}

		if err == nil && e.errorExpected {
			t.Errorf("%s: expected an error, but didn't get one", e.name)
		}
